...
function empty_html(ico, titulo, sub) {
  var iconName = ico && /^ic-[a-z-]+$/.test(ico) ? ico : 'ic-clipboard'
  return '<div class="empty"><div class="empty_ico"><svg class="ic" width="1.2em" height="1.2em"><use href="#'+iconName+'"/></svg></div><div class="empty_txt">'+(titulo||'Sem dados')+'</div>'+(sub?'<div class="empty_txt" style="font-size:12px;margin-top:4px">'+sub+'</div>':'')+'</div>'
}
...
function card_agendamento(a) {
  ...
  return '<div class="card_item">'
    ...
    +(a.barbearias?'<div class="card_item_nome"><svg class="ic" width="1.1em" height="1.1em"><use href="#ic-scissors"/></svg> '+esc(nome_barb)+'</div>':'')
    +'<div class="card_item_nome">'+esc(nome_cli)+'</div>'
    +'<div class="card_item_info">'+esc(data_txt)+' às '+esc(hora_txt)+esc(hora_fim_txt)+'</div>'
    +(tel?'<div class="card_item_info" style="color:var(--text2)"><svg class="ic" width="1.1em" height="1.1em"><use href="#ic-phone"/></svg> '+esc(tel)+'</div>':'')
